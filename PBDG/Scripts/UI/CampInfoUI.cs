...
public class CampInfoUI : IUserInterface
{
    ...
    private UnitCountVisitor m_UnitCountVisitor = new UnitCountVisitor(); //存货单位计数
    public CampInfoUI(BattleGame battleGame) : base(battleGame)
    {
        Initialize();
    }
    // 初始
    ...
    private void ShowOnTrainInfo()
    {
        ...
        m_battleGame.RunCharacterVisitor(m_UnitCountVisitor);
    ...
    private void OnLevelUpBtnClick()
    {
        ...
        if (CheckRule(m_battleGame.CostAP(Cost), Msg) == false)
            return;
    ...
    {
        if (bValue == false)
            m_battleGame.ShowGameMsg(NotifyMsg);
        return bValue;
    }
}
