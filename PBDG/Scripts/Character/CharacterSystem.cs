...
public class CharacterSystem : IGameSystem
{
    ...
    public CharacterSystem(BattleGame battleGame) : base(battleGame)
    {
        Initialize();

        // 注册事件
        m_battleGame.RegisterGameEvent(ENUM_GameEvent.NewStage, new NewStageObserverSoldierAddMedal(m_battleGame));
    }
    ...
    public void RemoveCharacter(List<ICharacter> Characters, List<ICharacter> Opponents, ENUM_GameEvent emEvent)
    {
        ...
        foreach (ICharacter Character in Characters)
        {
            ...
            if (Character.CheckKilledEvent() == false)
                m_battleGame.NotifyGameEvent(emEvent, Character);
