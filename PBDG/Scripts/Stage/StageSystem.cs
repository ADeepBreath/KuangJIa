...
public class StageSystem : IGameSystem
{
	...
	private int	m_EnemyKilledCount = 0;			// 目前敌方单位阵亡情况
	...
	public StageSystem(BattleGame battleGame):base(battleGame)
	{
		Initialize();
	}
	...
	public override void Initialize()
	{
		...
		m_NowStageLv = 1;
        // 注册游戏事件
        m_battleGame.RegisterGameEvent( ENUM_GameEvent.EnemyKilled, new EnemyKilledObserverStageScore(this)); 
	}
	...
	public override void Update()
	{
		...
		// 是否要切换下一关
		if(m_battleGame.GetEnemyCount() ==  0 )
		{
			// 是否结束
	...
	public void LoseHeart()
	{
        ...
        m_battleGame.ShowHeart( m_NowHeart );
	}

	// 增加目前击杀数(不通过GameEventSystem呼叫)
	public void AddEnemyKilledCount()
	{
		m_EnemyKilledCount++;
	}

    // 设定目前击杀数(通过GameEventSystem呼叫)
	...
	{
		m_battleGame.ShowGameMsg("新的关卡");
		m_NowStageLv++;

        //显示
        m_battleGame.ShowNowStageLv(m_NowStageLv);

        // 事件
        m_battleGame.NotifyGameEvent( ENUM_GameEvent.NewStage , m_NowStageLv );
